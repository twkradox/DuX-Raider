            ...
            for member in guild.members:
                if not member.bot:
                    try:
                        ...
                        print((Colorate.Color(Colors.green, f"[+] Message Sent to {member.name}#{member.discriminator} ({member.id}) - Time taken: {end_time_member - start_time_member:.2f} seconds")))
                        members_sent += 1
                    except Exception as e:
                        print((Colorate.Color(Colors.red, f"[-] Can't send message to {member.name}#{member.discriminator}: {e}")))
                        members_fail += 1
